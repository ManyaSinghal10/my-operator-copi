...
translation is performed by the local Gemma model,
and speech output is generated by the existing TTS engine.
...
- `backend/` - Python API server (`server.py` and `requirements.txt`) for Whisper STT, Gemma Translator, Piper TTS.
